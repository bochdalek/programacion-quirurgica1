...
export default createStore({
  state: {
    ...
    pacientesNoProgMedicacion: [
      ...
      { id: 7, nombre: 'Manuel Jiménez', medicacion: 'Ácido Acetilsalicílico', tiempoRestante: 0, tipoFractura: 'Cadera - Subcapital' }
    ],
  ...
  mutations: {
    ...
    moverANoProgramables(state, { paciente, tipo }) {
      if (tipo === 'medicacion') {
        ...
        // Antiagregantes
        if (paciente.medicacion && paciente.medicacion.aas) {
          tiempoRestante = 144; // 6 días en horas (promedio 5-7 días)
          medicacion = 'Ácido Acetilsalicílico';
        } else if (paciente.medicacion && paciente.medicacion.clopidogrel) {
          tiempoRestante = 144; // 6 días en horas (promedio 5-7 días)
          medicacion = 'Clopidogrel';
        } else if (paciente.medicacion && paciente.medicacion.prasugrel) {
          tiempoRestante = 204; // 8.5 días en horas (promedio 7-10 días)
          medicacion = 'Prasugrel';
        } else if (paciente.medicacion && paciente.medicacion.ticagrelor) {
          tiempoRestante = 120; // 5 días en horas
          medicacion = 'Ticagrelor';
        } 
        // Anticoagulantes
        else if (paciente.medicacion && paciente.medicacion.warfarina) {
          tiempoRestante = 120; // 5 días en horas
          medicacion = 'Warfarina';
        } else if (paciente.medicacion && paciente.medicacion.dabigatran) {
          tiempoRestante = 48; // 2 días en horas
          medicacion = 'Dabigatrán';
        } else if (paciente.medicacion && paciente.medicacion.rivaroxaban) {
          tiempoRestante = 48; // 2 días en horas
          medicacion = 'Rivaroxabán';
        } else if (paciente.medicacion && paciente.medicacion.apixaban) {
          tiempoRestante = 48; // 2 días en horas
          medicacion = 'Apixabán';
        } 
        // Otros
